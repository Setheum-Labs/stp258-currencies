...
fn minimum_balance_work() {
	ExtBuilder::default().build().execute_with(|| {
		assert_eq!(Stp258::minimum_balance(BTC), 1);
		assert_eq!(Stp258::minimum_balance(DOT), 2);
		assert_eq!(Stp258::minimum_balance(ETH), 0);
	});
}
...
fn is_module_account_id_work() {
	ExtBuilder::default().build().execute_with(|| {
		assert_eq!(Stp258::is_module_account_id(&ALICE), false);
		assert_eq!(Stp258::is_module_account_id(&BOB), false);
		assert_eq!(Stp258::is_module_account_id(&TREASURY_ACCOUNT), false);
		assert_eq!(Stp258::is_module_account_id(&DustAccount::get()), true);
	});
}
...
fn remove_dust_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(Stp258::deposit(DOT, &ALICE, 100));
		assert_eq!(Stp258::total_issuance(DOT), 100);
		assert_eq!(Accounts::<Runtime>::contains_key(ALICE, DOT), true);
		assert_eq!(Stp258::free_balance(DOT, &ALICE), 100);
		assert_eq!(System::providers(&ALICE), 1);
		assert_eq!(Accounts::<Runtime>::contains_key(DustAccount::get(), DOT), false);
		assert_eq!(Stp258::free_balance(DOT, &DustAccount::get()), 0);
		assert_eq!(System::providers(&DustAccount::get()), 0);

		// total is gte ED, will not handle dust
		assert_ok!(Stp258::withdraw(DOT, &ALICE, 98));
		assert_eq!(Stp258::total_issuance(DOT), 2);
		assert_eq!(Accounts::<Runtime>::contains_key(ALICE, DOT), true);
		assert_eq!(Stp258::free_balance(DOT, &ALICE), 2);
		assert_eq!(System::providers(&ALICE), 1);
		assert_eq!(Accounts::<Runtime>::contains_key(DustAccount::get(), DOT), false);
		assert_eq!(Stp258::free_balance(DOT, &DustAccount::get()), 0);
		assert_eq!(System::providers(&DustAccount::get()), 0);

		assert_ok!(Stp258::withdraw(DOT, &ALICE, 1));

		// total is lte ED, will handle dust
		assert_eq!(Stp258::total_issuance(DOT), 1);
		assert_eq!(Accounts::<Runtime>::contains_key(ALICE, DOT), false);
		assert_eq!(Stp258::free_balance(DOT, &ALICE), 0);
		...
		assert_eq!(Stp258::free_balance(DOT, &DustAccount::get()), 1);
		assert_eq!(System::providers(&DustAccount::get()), 1);

		let dust_lost_event = Event::stp258(crate::Event::DustLost(ALICE, DOT, 1));
...
fn set_lock_should_work() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			assert_ok!(Stp258::set_lock(ID_1, DOT, &ALICE, 10));
			assert_eq!(Stp258::accounts(&ALICE, DOT).frozen, 10);
			assert_eq!(Stp258::accounts(&ALICE, DOT).frozen(), 10);
			assert_eq!(Stp258::locks(ALICE, DOT).len(), 1);
			assert_ok!(Stp258::set_lock(ID_1, DOT, &ALICE, 50));
			assert_eq!(Stp258::accounts(&ALICE, DOT).frozen, 50);
			assert_eq!(Stp258::locks(ALICE, DOT).len(), 1);
			assert_ok!(Stp258::set_lock(ID_2, DOT, &ALICE, 60));
			assert_eq!(Stp258::accounts(&ALICE, DOT).frozen, 60);
			assert_eq!(Stp258::locks(ALICE, DOT).len(), 2);
		});
}
...
fn extend_lock_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(Stp258::locks(ALICE, DOT).len(), 1);
			assert_eq!(Stp258::accounts(&ALICE, DOT).frozen, 10);
			assert_ok!(Stp258::extend_lock(ID_1, DOT, &ALICE, 20));
			assert_eq!(Stp258::locks(ALICE, DOT).len(), 1);
			assert_eq!(Stp258::accounts(&ALICE, DOT).frozen, 20);
			assert_ok!(Stp258::extend_lock(ID_2, DOT, &ALICE, 10));
			assert_ok!(Stp258::extend_lock(ID_1, DOT, &ALICE, 20));
			assert_eq!(Stp258::locks(ALICE, DOT).len(), 2);
		});
}
...
fn remove_lock_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_ok!(Stp258::set_lock(ID_1, DOT, &ALICE, 10));
			assert_ok!(Stp258::set_lock(ID_2, DOT, &ALICE, 20));
			assert_eq!(Stp258::locks(ALICE, DOT).len(), 2);
			assert_ok!(Stp258::remove_lock(ID_2, DOT, &ALICE));
			assert_eq!(Stp258::locks(ALICE, DOT).len(), 1);
		});
}
...
fn frozen_can_limit_liquidity() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			assert_ok!(Stp258::set_lock(ID_1, DOT, &ALICE, 90));
			assert_noop!(
				<Stp258 as MultiCurrency<_>>::transfer(DOT, &ALICE, &BOB, 11),
				Error::<Runtime>::LiquidityRestrictions,
			);
			assert_ok!(Stp258::set_lock(ID_1, DOT, &ALICE, 10));
			assert_ok!(<Stp258 as MultiCurrency<_>>::transfer(DOT, &ALICE, &BOB, 11),);
		});
}
...
fn can_reserve_is_correct() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			assert_eq!(Stp258::can_reserve(DOT, &ALICE, 0), true);
			assert_eq!(Stp258::can_reserve(DOT, &ALICE, 101), false);
			assert_eq!(Stp258::can_reserve(DOT, &ALICE, 100), true);
		});
}
...
fn reserve_should_work() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			assert_noop!(Stp258::reserve(DOT, &ALICE, 101), Error::<Runtime>::BalanceTooLow,);
			assert_ok!(Stp258::reserve(DOT, &ALICE, 0));
			assert_eq!(Stp258::free_balance(DOT, &ALICE), 100);
			assert_eq!(Stp258::reserved_balance(DOT, &ALICE), 0);
			assert_eq!(Stp258::total_balance(DOT, &ALICE), 100);
			assert_ok!(Stp258::reserve(DOT, &ALICE, 50));
			assert_eq!(Stp258::free_balance(DOT, &ALICE), 50);
			assert_eq!(Stp258::reserved_balance(DOT, &ALICE), 50);
			assert_eq!(Stp258::total_balance(DOT, &ALICE), 100);
		});
}
...
fn unreserve_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_eq!(Stp258::free_balance(DOT, &ALICE), 100);
			assert_eq!(Stp258::reserved_balance(DOT, &ALICE), 0);
			assert_eq!(Stp258::unreserve(DOT, &ALICE, 0), 0);
			assert_eq!(Stp258::unreserve(DOT, &ALICE, 50), 50);
			assert_ok!(Stp258::reserve(DOT, &ALICE, 30));
			assert_eq!(Stp258::free_balance(DOT, &ALICE), 70);
			assert_eq!(Stp258::reserved_balance(DOT, &ALICE), 30);
			assert_eq!(Stp258::unreserve(DOT, &ALICE, 15), 0);
			assert_eq!(Stp258::free_balance(DOT, &ALICE), 85);
			assert_eq!(Stp258::reserved_balance(DOT, &ALICE), 15);
			assert_eq!(Stp258::unreserve(DOT, &ALICE, 30), 15);
			assert_eq!(Stp258::free_balance(DOT, &ALICE), 100);
			assert_eq!(Stp258::reserved_balance(DOT, &ALICE), 0);
		});
}
...
fn slash_reserved_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(Stp258::free_balance(DOT, &ALICE), 50);
			assert_eq!(Stp258::reserved_balance(DOT, &ALICE), 50);
			assert_eq!(Stp258::total_issuance(DOT), 200);
			assert_eq!(Stp258::slash_reserved(DOT, &ALICE, 0), 0);
			assert_eq!(Stp258::free_balance(DOT, &ALICE), 50);
			assert_eq!(Stp258::reserved_balance(DOT, &ALICE), 50);
			assert_eq!(Stp258::total_issuance(DOT), 200);
			assert_eq!(Stp258::slash_reserved(DOT, &ALICE, 100), 50);
			assert_eq!(Stp258::free_balance(DOT, &ALICE), 50);
			assert_eq!(Stp258::reserved_balance(DOT, &ALICE), 0);
			assert_eq!(Stp258::total_issuance(DOT), 150);
		});
}
...
fn repatriate_reserved_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(Stp258::reserved_balance(DOT, &ALICE), 0);
			assert_eq!(
				Stp258::repatriate_reserved(DOT, &ALICE, &ALICE, 0, BalanceStatus::Free),
				Ok(0)
			);
			assert_eq!(
				Stp258::repatriate_reserved(DOT, &ALICE, &ALICE, 50, BalanceStatus::Free),
				Ok(50)
			);
			assert_eq!(Stp258::free_balance(DOT, &ALICE), 100);
			assert_eq!(Stp258::reserved_balance(DOT, &ALICE), 0);

			assert_eq!(Stp258::free_balance(DOT, &BOB), 100);
			assert_eq!(Stp258::reserved_balance(DOT, &BOB), 0);
			assert_ok!(Stp258::reserve(DOT, &BOB, 50));
			assert_eq!(Stp258::free_balance(DOT, &BOB), 50);
			assert_eq!(Stp258::reserved_balance(DOT, &BOB), 50);
			assert_eq!(
				Stp258::repatriate_reserved(DOT, &BOB, &BOB, 60, BalanceStatus::Reserved),
				Ok(10)
			);
			assert_eq!(Stp258::free_balance(DOT, &BOB), 50);
			assert_eq!(Stp258::reserved_balance(DOT, &BOB), 50);
			...
			assert_eq!(
				Stp258::repatriate_reserved(DOT, &BOB, &ALICE, 30, BalanceStatus::Reserved),
				Ok(0)
			);
			assert_eq!(Stp258::free_balance(DOT, &ALICE), 100);
			assert_eq!(Stp258::reserved_balance(DOT, &ALICE), 30);
			assert_eq!(Stp258::free_balance(DOT, &BOB), 50);
			assert_eq!(Stp258::reserved_balance(DOT, &BOB), 20);

			assert_eq!(
				Stp258::repatriate_reserved(DOT, &BOB, &ALICE, 30, BalanceStatus::Free),
				Ok(10)
			);
			assert_eq!(Stp258::free_balance(DOT, &ALICE), 120);
			assert_eq!(Stp258::reserved_balance(DOT, &ALICE), 30);
			assert_eq!(Stp258::free_balance(DOT, &BOB), 50);
			assert_eq!(Stp258::reserved_balance(DOT, &BOB), 0);
		});
...
fn slash_draw_reserved_correct() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(Stp258::slash(DOT, &ALICE, 80), 0);
			assert_eq!(Stp258::free_balance(DOT, &ALICE), 0);
			assert_eq!(Stp258::reserved_balance(DOT, &ALICE), 20);
			assert_eq!(Stp258::total_issuance(DOT), 120);

			assert_eq!(Stp258::slash(DOT, &ALICE, 50), 30);
			assert_eq!(Stp258::free_balance(DOT, &ALICE), 0);
			assert_eq!(Stp258::reserved_balance(DOT, &ALICE), 0);
			assert_eq!(Stp258::total_issuance(DOT), 100);
		});
...
fn transfer_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(Stp258::transfer(Some(ALICE).into(), BOB, DOT, 50));
			assert_eq!(Stp258::free_balance(DOT, &ALICE), 50);
			assert_eq!(Stp258::free_balance(DOT, &BOB), 150);
			assert_eq!(Stp258::total_issuance(DOT), 200);

			let transferred_event = Event::stp258(crate::Event::Transferred(DOT, ALICE, BOB, 50));
			assert!(System::events().iter().any(|record| record.event == transferred_event));

			assert_noop!(
				Stp258::transfer(Some(ALICE).into(), BOB, DOT, 60),
...
fn transfer_all_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(Stp258::transfer_all(Some(ALICE).into(), BOB, DOT));
			assert_eq!(Stp258::free_balance(DOT, &ALICE), 0);
			assert_eq!(Stp258::free_balance(DOT, &BOB), 200);

			let transferred_event = Event::stp258(crate::Event::Transferred(DOT, ALICE, BOB, 100));
			assert!(System::events().iter().any(|record| record.event == transferred_event));
		});
}
...
fn deposit_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_ok!(Stp258::deposit(DOT, &ALICE, 100));
			assert_eq!(Stp258::free_balance(DOT, &ALICE), 200);
			assert_eq!(Stp258::total_issuance(DOT), 300);

			assert_noop!(
				Stp258::deposit(DOT, &ALICE, Balance::max_value()),
...
fn withdraw_should_work() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			assert_ok!(Stp258::withdraw(DOT, &ALICE, 50));
			assert_eq!(Stp258::free_balance(DOT, &ALICE), 50);
			assert_eq!(Stp258::total_issuance(DOT), 150);

			assert_noop!(Stp258::withdraw(DOT, &ALICE, 60), Error::<Runtime>::BalanceTooLow);
		});
}
...
fn slash_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(Stp258::slash(DOT, &ALICE, 50), 0);
			assert_eq!(Stp258::free_balance(DOT, &ALICE), 50);
			assert_eq!(Stp258::total_issuance(DOT), 150);

			// slashed_amount == amount
			assert_eq!(Stp258::slash(DOT, &ALICE, 51), 1);
			assert_eq!(Stp258::free_balance(DOT, &ALICE), 0);
			assert_eq!(Stp258::total_issuance(DOT), 100);
		});
}
...
fn update_balance_should_work() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			assert_ok!(Stp258::update_balance(DOT, &ALICE, 50));
			assert_eq!(Stp258::free_balance(DOT, &ALICE), 150);
			assert_eq!(Stp258::total_issuance(DOT), 250);

			assert_ok!(Stp258::update_balance(DOT, &BOB, -50));
			assert_eq!(Stp258::free_balance(DOT, &BOB), 50);
			assert_eq!(Stp258::total_issuance(DOT), 200);

			assert_noop!(Stp258::update_balance(DOT, &BOB, -60), Error::<Runtime>::BalanceTooLow);
		});
}
...
fn ensure_can_withdraw_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_noop!(
				Stp258::ensure_can_withdraw(DOT, &ALICE, 101),
				Error::<Runtime>::BalanceTooLow
			);

			assert_ok!(Stp258::ensure_can_withdraw(DOT, &ALICE, 1));
			assert_eq!(Stp258::free_balance(DOT, &ALICE), 100);
		});
}
...
fn no_op_if_amount_is_zero() {
	ExtBuilder::default().build().execute_with(|| {
		assert_ok!(Stp258::ensure_can_withdraw(DOT, &ALICE, 0));
		assert_ok!(Stp258::transfer(Some(ALICE).into(), BOB, DOT, 0));
		assert_ok!(Stp258::transfer(Some(ALICE).into(), ALICE, DOT, 0));
		assert_ok!(Stp258::deposit(DOT, &ALICE, 0));
		assert_ok!(Stp258::withdraw(DOT, &ALICE, 0));
		assert_eq!(Stp258::slash(DOT, &ALICE, 0), 0);
		assert_eq!(Stp258::slash(DOT, &ALICE, 1), 1);
		assert_ok!(Stp258::update_balance(DOT, &ALICE, 0));
	});
}
...
fn merge_account_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_eq!(Stp258::free_balance(DOT, &ALICE), 100);
			assert_eq!(Stp258::free_balance(BTC, &ALICE), 200);
			assert_eq!(Stp258::free_balance(DOT, &BOB), 0);

			assert_ok!(Stp258::reserve(DOT, &ALICE, 1));
			assert_noop!(
				Stp258::merge_account(&ALICE, &BOB),
				Error::<Runtime>::StillHasActiveReserved
			);
			Stp258::unreserve(DOT, &ALICE, 1);

			assert_ok!(Stp258::merge_account(&ALICE, &BOB));
			assert_eq!(Stp258::free_balance(DOT, &ALICE), 0);
			assert_eq!(Stp258::free_balance(BTC, &ALICE), 0);
			assert_eq!(Stp258::free_balance(DOT, &BOB), 100);
			assert_eq!(Stp258::free_balance(BTC, &BOB), 200);
		});
}
...
fn currency_adapter_ensure_currency_adapter_should_work() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			assert_eq!(Stp258::total_issuance(DOT), 102);
			assert_eq!(Stp258::total_balance(DOT, &Treasury::account_id()), 2);
			assert_eq!(Stp258::total_balance(DOT, &TREASURY_ACCOUNT), 100);
			assert_eq!(Stp258::reserved_balance(DOT, &TREASURY_ACCOUNT), 0);
			assert_eq!(Stp258::free_balance(DOT, &TREASURY_ACCOUNT), 100);
			assert_eq!(
...
fn currency_adapter_refunding_balance_should_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		Stp258::set_reserved_balance(DOT, &TREASURY_ACCOUNT, 69);
